...
export class ClubsComponent implements OnInit {
  ...
  guardar(){
    ...
    this.team.urlshirt = this.forma.get("camiseta")?.value;
      ...
      if (this.team.id === 0){
        this.apiServices.addTeamTemp(this.team).subscribe(
          response => {
            this.reset();
            this.forma.reset();
        ...
        this.apiServices.editTeamTemp(this.team).subscribe(
          response => {
            this.reset();
            this.forma.reset();
  ...
  buscador(){
    console.log('BUSCADOR:'+this.buscar + ", datosTeamAux:"+this.datosTeamsAux.length);
