...
@Component({
  selector: 'app-squadsadm',
  templateUrl: './squadsadm.component.html',
  styleUrls: ['./squadsadm.component.css']
})
export class SquadsadmComponent implements OnInit {
  ...
  constructor(private translate: TranslateService, private apiServices:ApifutfemService, private fb:FormBuilder, private futfemservice:FutfemserviceService) {
  ...
  eliminar(id:string){
    ...
    if (id != '0'){ // se trata de un alta
      this.apiServices.removeSquad(id).subscribe(
        response => {
  ...
  guardar():void{
    ...
    if (this.playerteam.id == 0){ // se trata de un alta
      this.apiServices.addSquad(this.playerteam).subscribe(
        response => {
          Swal.fire({
        ...
        this.apiServices.editSquad(this.playerteam).subscribe(
        response => {
          Swal.fire({
            title: this.translate.instant('FORM.OK.DONE'),
            icon: 'success',
  ...
  cargarPlantillas(idTeam:number, season:String){
    ...
    .subscribe( (datosPlayers : any) => {
      ...
      this.apiServices.getSquadsAllSeason(season)
      .subscribe( (datosPlayers : any) => {
