...
@Component({
  selector: 'app-refereesadm',
  templateUrl: './refereesadm.component.html',
  styleUrls: ['./refereesadm.component.css'],
  providers: [ UtilsPipe]
})
export class RefereesadmComponent implements OnInit {
  ...
  constructor( private utilPipe: UtilsPipe, private apiServices:ApifutfemService,private futfemservice:FutfemserviceService, private fb: FormBuilder, private translate:TranslateService) {
  ...
  cargarArbitros(){
    ...
    limpiar(){
      ...
      this.forma.get("casero")?.setValue(0);
      this.forma.get("tarjetero")?.setValue(0);
      this.forma.get("urlpic")?.setValue(0);
    ...
    guardar(  ) {
      ...
      if (this.referee.id === 0){
        this.apiServices.addReferee(this.referee).subscribe(
          response => {
          this.reset();
          this.forma.reset();
        ...
        this.apiServices.editReferee(this.referee).subscribe(
          response => {
          this.reset();
          this.forma.reset();
