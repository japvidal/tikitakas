...
@Component({
  selector: 'app-playersadm',
  templateUrl: './playersadm.component.html',
  styleUrls: ['./playersadm.component.css'],
  providers: [DatePipe, UtilsPipe]
})
export class PlayersadmComponent implements OnInit {
  ...
  guardar():void{
    ...
    this.player.portero = this.forma.get('portero')?.value;
      if (this.player.id == 0){
        this.apiServices.addPlayer(this.player).subscribe(
          response => {
            this.reset();
            Swal.fire( this.translate.instant('FORM.OK.DONE'), this.translate.instant('FORM.OK.MESSAGE'), 'success' );
           })
      }else{
        this.apiServices.editPlayer(this.player).subscribe(
          response => {
            this.reset();
